...
export default function Sidebar({
  ...
  return (
    <aside className="flex flex-col shrink-0 h-dvh overflow-hidden sidebar-responsive bg-brand-dark">
      ...
      {/* Nav body */}
      <div className="flex-1 flex flex-col min-h-0 p-3 gap-1 overflow-hidden">
        {/* New conversation */}
        <button
            ...
            <div className="flex flex-col gap-0.5">
              {history.map((conv) => {
                ...
                return (
                  <div
                    key={conv.id}
                    className={`group flex items-center gap-2 w-full px-3 py-2 rounded-xl text-left transition-colors cursor-pointer ${isActive
                    ...
                    tabIndex={0}
                    onKeyDown={(e) =>
                      e.key === "Enter" && onLoadConversation?.(conv)
                    }
                    title={conv.title}
                  >
                    {/* Chat bubble icon */}
