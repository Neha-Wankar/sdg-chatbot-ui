        ...
        return (
          ...
          >
            <div className="flex gap-2.5 items-center">
              <span
                className={`w-6 h-6 shrink-0 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                  selected
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-xs text-gray-900 leading-snug">{parts[0]}</div>
                {parts.slice(1).join(" → ") && (
                  <div className="text-[11px] text-gray-500 mt-0.5">{parts.slice(1).join(" → ")}</div>
                )}
              </div>
              {selected ? (
